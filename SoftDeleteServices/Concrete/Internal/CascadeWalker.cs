...
namespace SoftDeleteServices.Concrete.Internal {
    internal class CascadeWalker<TInterface>
        ...
        private class GenericDbSetLoader<TEntity> where TEntity : class {
            private readonly DbSet<TEntity> _dbSet;
            private readonly IEnumerable<TEntity> _query;

            public IEnumerable<TEntity> Get() {
                return _query;
            }

            public GenericDbSetLoader(DbContext context, Type entityType, string fkName, string entryPrimaryKey) {
                _dbSet = context.Set<TEntity>();

                ParameterExpression parameter = Expression.Parameter(entityType, "a");
                MemberExpression property = Expression.Property(parameter, fkName);
                BinaryExpression body = Expression.Equal(property, Expression.Constant(entryPrimaryKey));
                Expression<Func<TEntity, bool>> expression = Expression.Lambda<Func<TEntity, bool>>(body, parameter);

                _query = _dbSet.Where(expression);
                //.Where(x => entityType.GetProperty(fkName).GetValue(x).ToString() == entryPrimaryKey);
            }
        }
        ...
        public async ValueTask WalkEntitiesSoftDelete(object principalInstance, byte cascadeLevel) {
            ...
            var entry = _context.Entry(principalInstance);
            var entryPrimaryKey = entry.Metadata.FindPrimaryKey().Properties.Select(p => entry.Property(p.Name).CurrentValue).First();
            var referencingForeignKeys = entry.Metadata.GetDeclaredReferencingForeignKeys();
            foreach (var x in referencingForeignKeys) {
                var fkName = x.Properties.Select(p => p.Name).First();

                var genericHelperType =
                    typeof(GenericDbSetLoader<>).MakeGenericType(typeof(TInterface), x.DeclaringEntityType.ClrType);
                dynamic loader = Activator.CreateInstance(genericHelperType, _context, x.DeclaringEntityType.ClrType, fkName, entryPrimaryKey);
                var entities = loader.Get();

                if (entities != null) {
                    foreach(var entity in entities) {
                        var softDeleteInterface = _config.GetType().BaseType.GenericTypeArguments.First();
                        if (softDeleteInterface.IsAssignableFrom(entity.GetType())) {
                            ApplyChangeIfAppropriate((TInterface)entity, (byte)(cascadeLevel + 1));
            ...
            foreach (var navigation in principalNavs) {
                ...
                if (navigation.IsCollection) {
                    ...
                    foreach (var entity in navValue as IEnumerable) {
                        if (_stopCircularLook.Contains(entity)) continue; // already checked by foreign key rather than navigation
                    ...
                    if (navValue == null)
                        return; //no relationship
                    if (_stopCircularLook.Contains(navValue)) continue; // already checked by foreign key rather than navigation
